...
Rectangle {
    ...
    function openVideoPlayer(source, title, pid) {
        ...
        if (player) {
            ...
            if (pid) {
                options.pid = pid
                console.log("openVideoPlayer: 传入 pid =", pid)
            }
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    property var mainNewsItem: {
                        ...
                        // 找到第一条非完整版新闻
                        for (var i = 0; i < newsManager.news.length; i++) {
                            if (!newsManager.news[i].isFullVersion) {
                                return newsManager.news[i]
                            }
                        }
                        // 如果所有新闻都是完整版，显示第一条
                        return newsManager.news[0]
                    }
