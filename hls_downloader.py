        ...
        try:
            ...
            try:
                ...
                for segment_url in segments:
                    ...
                    try:
                        # 使用 HEAD 请求获取片段大小
                        head_response = self.session.head(segment_url, timeout=10)
                        segment_size = int(head_response.headers.get('content-length', 0))
                        if segment_size > 0:
                            total_size += segment_size
                            segment_info.append((segment_url, segment_size))
                        else:
                            # 如果无法获取大小，使用默认值并稍后更新
                            segment_info.append((segment_url, 0))
                            total_size += 500 * 1024  # 假设 500KB
                    except:
                        # 获取失败时使用默认值
                        segment_info.append((segment_url, 0))
                        total_size += 500 * 1024
                ...
                for i, (segment_url, estimated_size) in enumerate(segment_info):
                    if self._cancelled:
                        return False

                    segment_file = os.path.join(temp_dir, f"segment_{i:06d}.ts")
                    actual_size = self._download_segment(segment_url, segment_file)
                    ...
                    if self.progress_callback:
                        # 如果总大小是准确的（所有片段都有实际大小）
                        if all(size > 0 for _, size in segment_info):
                            progress = int(downloaded_size / total_size * 100)
                        else:
                            # 否则使用片段数量
                            progress = int((i + 1) / len(segment_info) * 100)
                        self.progress_callback(progress, i + 1, len(segment_info))
